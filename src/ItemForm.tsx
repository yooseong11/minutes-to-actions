...
export interface ItemValues { content: string; type: ItemType; assignee: RawAttendee | null; due: string | null }
export default function ItemForm({ initial, attendees, onSave, onCancel, adding = false }: {
  ...
  return (
    <form className="item-form" onSubmit={event => {
      ...
      onSave({ content: content.trim(), type, due: due || null,
      ...
      <fieldset className="edit-assignee">
        ...
        <div className="edit-row">
          ...
          <button type="button" className="button button--quiet" onClick={() => { setName(''); setContext('') }}>담당자 지우기</button>
        </div>
      </fieldset>
      ...
      <div className="edit-row">
        ...
        <button type="button" className="button button--quiet" onClick={() => setDue('')}>기한 지우기</button>
      </div>
      {error && <p className="error" role="alert">{error}</p>}
      <div className="actions">
        <button type="submit" className="button">{adding ? '안건 추가' : '수정 적용'}</button>
        <button type="button" className="button button--quiet" onClick={onCancel}>취소</button>
      </div>
    </form>
  )
}
