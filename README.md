...
### 어떤 형태로 추출할까?
- 해결 방법
  - **가상의 사용자를 만들어서 사용자 시나리오를 만들었습니다.** 1명은 회의 참석자, 1명은 팀장(PM)으로 정했습니다. 회의 참석자는 회의 직후에 회의 내용을 당일 공유하기 위해서 요약본을 노션, 슬랙, 구글 스프레드 시트에 업로드 한다고 가정했습니다. 팀장은 몇 주간 걸쳐서 일어난 회의록을 확인하며 결정, 할일, 미결 문제를 파악한다고 가정했습니다
  - 결과
    - 마크다운: 당일 회의 참석자가 노션, 메일, 슬랙 등으로 공유하기엔 마크다운 형식이 최선이라고 판단했습니다.
    - CSV: 팀장이 나중에 회의록을 모아서 한눈에 볼 때, 엑셀이나 구글 스프레드를 이용하여 분류에 필터를 걸어서 원하는 대로 보는 것이 가능하도록 했습니다.
...
### 회의 내용이 외부 AI 회사로 전송되나요? 보안 정책상 외부 AI API를 쓸 수 없는 회사도 사용할 수 있나요?

- 현재 OpenAI API로 회의록을 전송만 하고 서버에 저장하지 않습니다. 즉, 웹 개발자인 저에게 보여지는 내용은 하나도 없습니다. 여기서 더 나아가 OpenAI에 전송조차 불가능 하다면 Ollma나 Azure 같은 서비스를 선택하는 것도 가능합니다. 현재 API 호출을 한 곳으로 모아둬서 LLM 종류를 변경하는 건 어렵지 않습니다.

### 회사 회의록 양식에 맞게 추출 형식을 바꿀 수 있나요?

- 네, 쉽게 바꿀 수 있습니다. 요약된 회의록을 마크다운으로 변환하는 작업은 AI가 아니라 코드가 합니다.  코드만 수정하면 되는 작업이라 간단하게 가능합니다.

### 700자보다 긴 회의록도 요약할 수 있나요?

-기술적으로 가능합니다. 현재 회의록 요약을 700자까지로 제한한 이유는 비용 때문입니다. 회의록을 요약할 때마다 AI API 호출이 일어나 사용료가 나갑니다. input 데이터가 많아질 수록 output 데이터도 늘어납니다. 데이터가 많아지면 포트폴리오용 프로젝트에서 비용을 감당하기 힘들어져서 내린 한도입니다.